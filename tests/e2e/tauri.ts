...
async function connectWithRetry(port: number, timeoutMs: number): Promise<Browser> {
  ...
  while (Date.now() < deadline) {
    try {
      return await chromium.connectOverCDP(`http://localhost:${port}`, { timeout: 10_000 });
    } catch (e) {
      lastErr = e;
...
async function findTauriPage(browser: Browser, timeoutMs: number): Promise<Page> {
  ...
  throw new Error("Tauri WebView page (tauri://) not found via CDP");
}
...
export async function launchApp(): Promise<LaunchedApp> {
  ...
  try {
    const browser = await connectWithRetry(port, 60_000);
